...
class JobSchedule extends GetWidget<JobController> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<JobController>(builder: (controller)=> AppTabPanel(
      children: [
        const Text('Schedule', style: AppStyles.headerRegular),
        if (controller.jobModel!.scheduleModels.isEmpty && !_isAddScheduleOpen.value)
        ...
        Column(
          children: controller.jobModel!.scheduleModels.map((schedule) =>
              ScheduleCard(scheduleId: schedule.objectId)).toList(),
        ),
        Visibility(
